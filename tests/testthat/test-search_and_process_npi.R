...
# Test case for NPI search and processing
test_that("Processes various names correctly", {
  cat("Running test: Processes various names correctly\n")
  ...
  for (test_case in test_cases) {
    ...
    if (test_case$first == "Invalid") {
      # If the name is "Invalid", expect no results
      expect_equal(nrow(result), 0)
    } else {
      # For valid names, expect at least one row with relevant columns
      expect_true(nrow(result) >= 1)
      expect_true("basic_first_name" %in% colnames(result))
      expect_true("basic_last_name" %in% colnames(result))
      expect_true("taxonomies_desc" %in% colnames(result))
    }
  }
})
