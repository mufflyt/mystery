#' Query CMS Facility Affiliations
#'
#' @description
#' Fetches facility affiliation data from the CMS Provider Data Catalog API. Returns
#' facility affiliations like hospitals, nursing homes, rehab facilities, etc.
#'
...
#' @param url Character. API endpoint URL. Default: CMS data catalog API endpoint
...
#'
#' @return A tibble containing facility affiliation data with "FAC_" prefixed columns.
#'   Common returned fields include:
#'   \itemize{
#'     \item FAC_npi - National Provider Identifier
#'     \item FAC_ind_pac_id - Individual PAC ID
#'     \item FAC_provider_name - Provider's name
#'     \item FAC_facility_type - Type of facility (Hospital, Nursing Home, etc.)
#'     \item FAC_certification_number - Facility certification number
#'   }
#'
#' @examples
#' # Basic usage with minimal parameters
#' fac_results <- get_facility_affiliations(
#'   npi_numbers = c("1234567890", "0987654321"),
#'   verbose = TRUE
#' )
#'
#' # Example with batch processing
#' fac_with_batches <- get_facility_affiliations(
...
#' )
#'
#' # Full example with all parameters and CSV output
#' fac_full <- get_facility_affiliations(
#'   npi_numbers = c("1234567890", "0987654321"),
#'   batch_size = 5,
...
#'
#' @export
phase0_get_facility_affiliations <- function(
    npi_numbers,
    batch_size = 10,
    url = "https://data.cms.gov/provider-data/api/1/datastore/query/27ea-46a8/0",
    limit = 100,
  ...
  if (verbose) {
    tryCatch({
      logger::log_info("Starting facility affiliations query function...")
...
save_records_to_csv <- function(combined_records, csv_save_path, verbose) {
  if (!is.null(csv_save_path) && nrow(combined_records) > 0) {
    tryCatch({
      ...
      file_path <- file.path(
        csv_save_path,
        paste0("cms_facility_data_", timestamp, ".csv")
      )
