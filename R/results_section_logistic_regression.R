...
#' @param predictor_vars A vector of strings representing the names of predictor variables.
#' @param significance_level A numeric value specifying the significance level for filtering predictors.
#' @return A data frame containing significant predictors with their p-values and formatted p-values.
...
logistic_regression <- function(df, target_variable, predictor_vars, significance_level) {
  ...
  for (var in predictor_vars) {
    ...
    if (length(unique(df[[var]])) > 1) {
      ...
      cat("Skipping predictor", var, "due to insufficient unique values.\n")
    }
  }

  # Format the p-values
  results <- results %>%
    dplyr::mutate(Formatted_P_Value = ifelse(P_Value < 0.01, "<0.01", round(P_Value, 2)))
