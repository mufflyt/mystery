...
#'
#' Cleans, processes, and prepares the Phase 1 results data for further analysis.
#' The function allows row replication, optional label assignment, and saving the cleaned data to a CSV file.
#'
#' @param phase1_input A data frame containing Phase 1 results data.
#' @param replicate_rows An optional integer specifying how many times to replicate the rows (e.g., 2, 3, 4). Default is NULL (no replication).
#' @param replication_labels An optional character vector of labels (e.g., races) to assign to the replicated rows. Default is NULL.
#' @param csv_file_path An optional path to save the cleaned data as a CSV file. If NULL, the file is saved with a timestamp in the `cleaned_data` directory.
#'
#' @return The cleaned data frame after processing.
#'
#' @examples
#' \dontrun{
#' cleaned_phase1 <- clean_phase_1_results(my_data, 4, c("White", "Black", "Asian", "HIPI"), "output.csv")
#' }
#'
#' @export
