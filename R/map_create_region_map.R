...
#' @noRd
validate_inputs <- function(remove_ak_hi,
                            districts_per_group,
                            alpha_level,
                            save_path) {
  ...
  if (!districts_per_group %in% c("acog_districts",
                                  "ENT_Board_of_Governors_Regions",
                                  "US_Census_Subdivisions")) {
    logger::log_error(
      "Invalid 'districts_per_group': Must be one of 'acog_districts',
      'ENT_Board_of_Governors_Regions', or 'US_Census_Subdivisions'."
    )
    stop(
      "Invalid 'districts_per_group': Must be one of 'acog_districts',
      'ENT_Board_of_Governors_Regions', or 'US_Census_Subdivisions'."
  ...
  if (!is.numeric(alpha_level) ||
      alpha_level < 0 || alpha_level > 1) {
    logger::log_error("Invalid 'alpha_level': Must be a numeric value
                      between 0 and 1.")
...
join_with_acog_or_region <- function(states_sf, districts_per_group) {
  if (districts_per_group == "acog_districts") {
    ...
    joined_states <- dplyr::left_join(states_sf, ACOG_Districts_sf, by =
                                        c("name" = "State"))
    ...
    logger::log_info("Joining ENT Board of Governors Regions data
                     with U.S. states shapefile...")
    joined_states <- dplyr::left_join(states_sf, ent_bog_regions,
                                      by = c("name" = "State"))
    fill_column <- "ENT_BOG_Region"
    logger::log_success("Joined with ENT Board of Governors Regions.")
  } else {
    logger::log_info("Joining US Census Subdivisions with
                     U.S. states shapefile...")
    joined_states <- dplyr::left_join(states_sf, regions_df,
                                      by = c("name" = "State"))
...
generate_color_palette <- function(states_data, fill_column) {
  ...
  logger::log_info("Generating a color palette for
                   {num_fill_values} unique values...")
  color_palette <- viridis::viridis(num_fill_values, option = "D",
                                    direction = 1)
...
#' @noRd
create_map_plot <- function(states_data,
                            fill_column,
                            color_palette,
                            alpha_level,
                            title,
  ...
  map_plot <- ggplot2::ggplot() +
    ...
    ggplot2::scale_fill_manual(values =
                                 setNames(color_palette,
                                  unique(states_data[[fill_column]])),
                               na.value = "grey80")
...
#' Main function to create a map of U.S. States by Region, Division, or
#' Custom Districts
#'
#' This function generates a map of U.S. states, coloring them by regions,
#' divisions, ACOG districts,
#' or ENT Board of Governors Regions. Users can customize which grouping to
#' use (ACOG Districts,
#' ENT_Board_of_Governors_Regions, or US Census Subdivisions).
#'
#' @param remove_ak_hi Logical, whether to exclude Alaska and Hawaii from the
#' map. Default is TRUE.
#' @param districts_per_group Character, which grouping to use for the regions.
#' Options are:
#'   "acog_districts", "ENT_Board_of_Governors_Regions", or
#'   "US_Census_Subdivisions".
#' @param save_path Character, an optional file path to save the map image.
#' If NULL, the map will not
#'   be saved. Default is NULL.
#' @param alpha_level Numeric, the transparency level for the map's fill color,
#'  with 0 being fully
#'   transparent and 1 fully opaque. Default is 0.2.
#' @param title Character, the title for the map. Default is "U.S. States by
#' Region/Division or Custom
#'   Districts".
#' @param subtitle Character, an optional subtitle for the map.
#'
#' @return A ggplot map object representing the U.S. states colored by
#' region/division, ACOG districts,
#' or ENT_Board_of_Governors_Regions.
#'
#' @examples
...
# create_region_map(remove_ak_hi = TRUE, districts_per_group =
# "ENT_Board_of_Governors_Regions", save_path = NULL, alpha_level = 0.4,
# title = "U.S. States by Region/Division or Custom Districts", subtitle = NULL)
